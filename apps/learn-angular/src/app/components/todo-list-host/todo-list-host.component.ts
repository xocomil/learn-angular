...
import { selectListIds, todoLists } from '../../+store/todos.selectors';
...
@Component({
  ...
  template: `
    <learn-angular-todo *ngFor="let listId of listIds$ | ngrxPush" [todoListId]="listId"></learn-angular-todo>
    <pre>{{ todoLists$ | ngrxPush | json }}</pre>
  `,
...
export default class TodoListHostComponent {
  ...
  protected readonly todoLists$ = this.#store.select(todoLists);
}
